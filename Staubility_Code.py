...
def prediction(input_features):
    ...
    print("Load weights: {}sec".format(time.time()-start))
    start_1 = time.time()
    y_pred = loaded_model.predict(test_set)
    print("predictor time: {}sec, total time:{}, total genes:{}".format(time.time() - start_1,time.time()-start,len(list(y_pred))))
    y_top = np.array(y_pred).argsort()[::-1]#[:10]
    top_names = [names[x] for x in y_top]

    new_data =pd.DataFrame(top_names)
    new_data.to_csv("top_prediction.csv")
...
def step_1(target_seq,gene_data_list_path= join(getcwd(), r'In\utils\orf_coding_all.fasta.gz'),
    ...
    print("First step time = {}".format(time.time()-start))
...
def step_2(target_seq,genes_sequence_path= join(getcwd(), r'In\utils\seq_orf.csv'),
    ...
    ret = choose_linker(type_of_linker,target_gene_aa,essen_gene_aa,output_path,linkers)
...
def step_3(target_seq,genes_sequence_path= join(getcwd(), r'In\utils\seq_orf.csv'),essen_gene_orf="",linker_aa="",
    ...
    print("Third step time = {}".format(time.time() - start))
    return "Success!"
